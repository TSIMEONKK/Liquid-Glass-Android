...
- **🧩 Ready-made Widgets** - `LiquidGlassButton` / `LiquidGlassFab` / `LiquidGlassTabBar` / `LiquidGlassTabLayout` / `LiquidGlassChip` + `LiquidGlassChipGroup` / `LiquidGlassListItem` + `LiquidGlassListGroup` / `LiquidGlassToast` / `LiquidGlassDialogBuilder`: pre-wired glass button, FAB, tab bar, Material-style tabs with ViewPager2 sync, chips (filter / input), M3 list (merged panel or separated cards, built-in or your own row content), toast and Material dialog with click handling and adaptive foreground colors
...
| `LiquidGlassTabLayout` | Material `TabLayout`-style tab strip on one glass pill. The selection indicator is the same refracting glass droplet as `LiquidGlassTabBar`: it slides with overshoot and liquid stretch, and its width morphs between tabs of different widths. `MODE_FIXED` (equal widths, default) or `MODE_SCROLLABLE` (content-sized tabs that scroll, selected tab auto-centred). Kotlin: `newTab()` / `addTab(tab)` / `addTab(text)`, `Tab.text` / `Tab.icon` / `Tab.select()`, `selectTab(index)`, `selectedTabPosition`, `tabCount`, `setScrollPosition(position, offset)` for pager sync, `addOnTabSelectedListener`, `addOnTabReselectedListener`, `tabMode`, `selectedTintColor`. XML: `app:glassTabEntries`, `app:glassTabMode` |
| `LiquidGlassTabLayoutMediator` | Binds a `LiquidGlassTabLayout` to a `ViewPager2`, same usage as Material's `TabLayoutMediator`: `LiquidGlassTabLayoutMediator(tabLayout, viewPager) { tab, position -> tab.text = titles[position] }.attach()`. The droplet follows swipe progress, tapping a tab pages the pager, and tabs are rebuilt when the adapter changes (`autoRefresh`). ViewPager2 is `compileOnly` in this library: add `androidx.viewpager2` to your app |
| `LiquidGlassChip` | 32dp pill chip: optional leading icon, text, optional close icon. Filter chip: `isCheckable` makes a tap toggle `isChecked`; checked chips take `checkedTint` (default 40% iOS blue) and show a check in the leading slot. Input chip: `isCloseIconVisible` + `setOnCloseIconClickListener`. XML: `android:text`, `android:textSize`, `android:textColor`, `android:checkable`, `android:checked`, `app:glassChipIcon`, `app:glassCloseIconVisible`, `app:glassCheckedIconVisible`, `app:glassCheckedTint`. Kotlin: `text`, `chipIcon`, `isChipIconTintEnabled`, `isCheckable`, `isChecked`, `setOnCheckedChangeListener`, `checkedTint`, `isCloseIconVisible`, `closeIcon`, `setTextColor(color)`, `textView` |
| `LiquidGlassChipGroup` | Flow container for chips, like Material's `ChipGroup`: wraps onto new lines (`chipSpacingHorizontal` / `chipSpacingVertical`) or stays on one line (`isSingleLine`, inside a `HorizontalScrollView`). `isSingleSelection`, `isSelectionRequired`, `checkedChipIds` / `checkedChipId`, `check(id)`, `clearCheck()`, `setOnCheckedStateChangeListener`. Like `LiquidGlassListGroup` it is a transparent container: set `backdropSource` on the group and it is pushed to every chip. XML: `app:glassChipSpacingHorizontal`, `app:glassChipSpacingVertical`, `app:glassSingleLine`, `app:glassSingleSelection`, `app:glassSelectionRequired`, `app:backdropSourceId` |
| `LiquidGlassListItem` | Material 3 list item: leading icon, headline, supporting text, trailing text / icon, 56dp / 72dp min height. `position` (`SINGLE` / `FIRST` / `MIDDLE` / `LAST`) decides which corners round and which edges are flat, so stacked rows read as one panel. `expandedView` gives tap-to-expand with an animated height change. Custom content (the Material 3 `ListItemCardView` pattern): child views written inside `<LiquidGlassListItem>`, or `contentView` in code, replace the built-in row, and the item only draws the position-shaped glass and keeps expand / click. XML: `app:glassHeadline`, `app:glassSupportingText`, `app:glassLeadingIcon`, `app:glassTrailingIcon`, `app:glassTrailingText`, `app:glassListPosition`, `app:glassGroupCornerRadius`, `app:glassInnerCornerRadius`. Kotlin: `headline`, `supportingText`, `leadingIcon`, `trailingIcon`, `trailingText`, `position`, `expandedView`, `setExpanded()`, `contentView`, `updatePosition(index, count)` / `positionFor(index, count)` for RecyclerView adapters, `applyGroupPositions(items)` |
...
| `LiquidGlassToast` | Toast-shaped glass message in **its own window, stacked above the activity and any open dialog or bottom sheet** (a non-touchable, non-focusable app window, not a `Toast` window). Its backdrop is composed from the app's windows beneath it (the activity, dialogs and their dim), so it refracts whatever it covers, and it is unaffected by the API 30 custom-toast deprecation. A dialog opened while it is showing does not cover it; with the keyboard up it sits above the keyboard. `makeText(context, text, duration).show()`; `LENGTH_SHORT` / `LENGTH_LONG` equal `Toast`'s. `setIcon(drawable)` / `setIconResource(id)`, `setIconTintEnabled(false)` for full-colour icons, `setGravity(gravity, xOffset, yOffset)`, `durationMillis`, `setTextColor(color)`, `cancel()`, `cancelCurrent()`. Exposes `glass`, `textView`, `imageView`. Needs an Activity context (a dialog's context also works); one toast at a time, a new `show()` replaces the previous one; removed when the activity is destroyed |
...
`com.github.QWEA0:Liquid-Glass-Android:v2.0.11` — either resolves to the library. The
...
- `LiquidGlassButton` / `LiquidGlassFab` / `LiquidGlassTabBar` / `LiquidGlassTabLayout` / `LiquidGlassChip` / `LiquidGlassListItem` - Ready-made widgets built on `LiquidGlassView`
- `LiquidGlassListGroup` - Container for list items: merged panel or separated cards, positions assigned automatically
- `LiquidGlassChipGroup` - Flow container for chips: wrapping or single line, single / multi selection
- `LiquidGlassTabLayoutMediator` - Binds a `LiquidGlassTabLayout` to a ViewPager2
- `LiquidGlassToast` - Glass toast in its own window, above dialogs and bottom sheets
...
- **🔧 高度可定制** - 精细调节玻璃效果的每个方面
- **🧩 现成小部件** - `LiquidGlassButton` / `LiquidGlassFab` / `LiquidGlassTabBar` / `LiquidGlassTabLayout` / `LiquidGlassChip` + `LiquidGlassChipGroup` / `LiquidGlassListItem` + `LiquidGlassListGroup` / `LiquidGlassToast` / `LiquidGlassDialogBuilder`：预置点击派发与前景自适应配色的玻璃按钮、悬浮按钮、标签条、Material 式标签页（可与 ViewPager2 联动）、Chip（筛选 / 输入）、M3 列表（合并面板或分离卡片，内置行或自带内容）、Toast 和 Material 弹窗
...
| `LiquidGlassTabLayout` | Material `TabLayout` 那一类的横向标签条，整条是一块玻璃。选中指示与 `LiquidGlassTabBar` 同一种会折射的玻璃滴：切换时带过冲和液态拉伸滑过去，标签宽度不同时宽度跟着插值。`MODE_FIXED`（平分宽度，默认）或 `MODE_SCROLLABLE`（按内容宽度排开、可横向滑动，选中项自动滚到中间）。Kotlin：`newTab()` / `addTab(tab)` / `addTab(text)`、`Tab.text` / `Tab.icon` / `Tab.select()`、`selectTab(index)`、`selectedTabPosition`、`tabCount`、跟翻页进度的 `setScrollPosition(position, offset)`、`addOnTabSelectedListener`、`addOnTabReselectedListener`、`tabMode`、`selectedTintColor`。XML：`app:glassTabEntries`、`app:glassTabMode` |
| `LiquidGlassTabLayoutMediator` | 把 `LiquidGlassTabLayout` 和 `ViewPager2` 绑在一起，用法同 Material 的 `TabLayoutMediator`：`LiquidGlassTabLayoutMediator(tabLayout, viewPager) { tab, position -> tab.text = titles[position] }.attach()`。拖动翻页时玻璃滴跟着进度走，点标签翻页，adapter 数据变了自动重建标签（`autoRefresh`）。本库对 ViewPager2 是 `compileOnly`，应用需自己依赖 `androidx.viewpager2` |
| `LiquidGlassChip` | 32dp 胶囊 chip：前置图标（可选）、文字、关闭图标（可选）。筛选 chip：`isCheckable` 后点击切换 `isChecked`，选中时玻璃染成 `checkedTint`（默认 40% 的 iOS 系统蓝）、前置位置显示对勾。输入 chip：`isCloseIconVisible` + `setOnCloseIconClickListener`。XML：`android:text`、`android:textSize`、`android:textColor`、`android:checkable`、`android:checked`、`app:glassChipIcon`、`app:glassCloseIconVisible`、`app:glassCheckedIconVisible`、`app:glassCheckedTint`。Kotlin：`text`、`chipIcon`、`isChipIconTintEnabled`、`isCheckable`、`isChecked`、`setOnCheckedChangeListener`、`checkedTint`、`isCloseIconVisible`、`closeIcon`、`setTextColor(color)`、`textView` |
| `LiquidGlassChipGroup` | 装 chip 的流式容器，对应 Material 的 `ChipGroup`：放不下自动换行（`chipSpacingHorizontal` / `chipSpacingVertical`），或 `isSingleLine` 排成一行（外面套 `HorizontalScrollView`）。`isSingleSelection`、`isSelectionRequired`、`checkedChipIds` / `checkedChipId`、`check(id)`、`clearCheck()`、`setOnCheckedStateChangeListener`。与 `LiquidGlassListGroup` 一样是透明容器：`backdropSource` 设在组上，下发到每个 chip。XML：`app:glassChipSpacingHorizontal`、`app:glassChipSpacingVertical`、`app:glassSingleLine`、`app:glassSingleSelection`、`app:glassSelectionRequired`、`app:backdropSourceId` |
| `LiquidGlassListItem` | Material 3 列表项：前导图标、标题、辅助文字、尾部文字/图标，最小高度 56dp / 72dp。`position`（`SINGLE` / `FIRST` / `MIDDLE` / `LAST`）决定圆哪几个角、哪几条边是平边，几行叠起来就是一块面板。`expandedView` 提供点击展开，高度带动画。自带内容（Material 3 `ListItemCardView` 的用法）：写在 `<LiquidGlassListItem>` 里面的子视图、或代码里的 `contentView` 会替换内置行，行只负责按位置画玻璃形状，展开 / 点击照旧。XML：`app:glassHeadline`、`app:glassSupportingText`、`app:glassLeadingIcon`、`app:glassTrailingIcon`、`app:glassTrailingText`、`app:glassListPosition`、`app:glassGroupCornerRadius`、`app:glassInnerCornerRadius`。Kotlin：`headline`、`supportingText`、`leadingIcon`、`trailingIcon`、`trailingText`、`position`、`expandedView`、`setExpanded()`、`contentView`、`updatePosition(index, count)` / `positionFor(index, count)`（给 RecyclerView 适配器用）、`applyGroupPositions(items)` |
...
| `LiquidGlassToast` | Toast 形状的玻璃消息条，**自带一个 window，叠在 Activity 和打开着的弹窗、底部弹层之上**（不可触摸、不抢焦点的应用 window，不是 `Toast` window）。玻璃的背景由底下各层 window（Activity、弹窗及其变暗层）拼成，折射的就是它盖住的画面，也不受 API 30 自定义 Toast 废弃的影响。显示期间再打开的弹窗不会盖住它；键盘弹出时落在键盘上方。`makeText(context, text, duration).show()`；`LENGTH_SHORT` / `LENGTH_LONG` 与 `Toast` 相同。`setIcon(drawable)` / `setIconResource(id)`、彩色图标用 `setIconTintEnabled(false)`、`setGravity(gravity, xOffset, yOffset)`、`durationMillis`、`setTextColor(color)`、`cancel()`、`cancelCurrent()`。暴露 `glass`、`textView`、`imageView`。需要 Activity 的 Context（传弹窗的 context 也行）；同时只显示一条，再次 `show()` 顶掉上一条；Activity 销毁时随之移除 |
...
同一个 AAR 还有一份别名 `com.github.QWEA0:Liquid-Glass-Android:v2.0.11`，两者等价。
...
- `LiquidGlassButton` / `LiquidGlassFab` / `LiquidGlassTabBar` / `LiquidGlassTabLayout` / `LiquidGlassChip` / `LiquidGlassListItem` - 基于 `LiquidGlassView` 的现成小部件
- `LiquidGlassListGroup` - 列表项容器：合并面板或分离卡片，位置自动分配
- `LiquidGlassChipGroup` - chip 的流式容器：换行或单行，单选 / 多选
- `LiquidGlassTabLayoutMediator` - 把 `LiquidGlassTabLayout` 和 ViewPager2 绑在一起
- `LiquidGlassToast` - 自带 window、叠在弹窗和底部弹层之上的玻璃 Toast
